  ...
  export default {
    ...
    methods:{
      getTwdAndShortLinkInfo:function(type){
        ...
        const obj = this
        fetch('POST', 'quergetTwdAndShortLinkInfo', params).then(res=>{
          if(!res.success){
            alert("调用接口失败");
            return;
          }
          if(res.message == 'successful'){
            ...
            if (a.indexOf('wechat') > -1 || a.indexOf("MicroMessenger") > -1) {
              obj.jumpUrl = "http://www.tooklili.com:81/taobao?backurl=" + encodeURIComponent(res.data.couponLink);
            } else {
      ...
      superSearch:function(){
        const obj = this;
        MessageBox.confirm('是否使用超级搜索查询本商品更多优惠？','确认操作').then(action => {
      ...
      toggleShow:function(){
        if(!this.show){
          setTimeout(function(){
            $("html,body").animate({scrollTop:$(".goodsshow").offset().top},300);
          }, 500);
      ...
      getItemImg:function(){
        ...
        const obj = this
        fetch('POST', 'querygetItemImages', params).then(res=>{
          if(res.message == 'successful'){
      ...
      getItemDetail:function(){
        if(this.get_itemDetails_id!= null &&this.get_itemDetails_id!= 'null' && this.get_itemDetails_id !=''){
      ...
      getCamera:function(){
        ...
        fetch('POST', 'querygetQrCodeBase64', params).then(res=>{
      ...
      getRandomItemByCateId:function(){
        ...
        obj.url = 'querygetRandomItemByCateId',
          obj.params = par;
